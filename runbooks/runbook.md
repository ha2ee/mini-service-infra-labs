...
- `/health`가 200이어도 `/ready`는 503일 수 있다.
- 현재 실습 환경에서는 `LAB_ENV`를 설정하면 `/ready`가 200으로 바뀔 수 있다.
...
- 현재 실습 환경에서는 `LAB_ENV` 미설정 시 `/ready`가 503을 반환한다.
- `LAB_ENV`를 설정한 뒤 App을 다시 실행하면 `/ready`가 200으로 바뀔 수 있다.
